$(document).ready(function () {
	...
	var tabel_rekap_hotel_kota = $('#tabel_rekap_hotel_kota').DataTable({
        "processing"        : true,
        "serverSide"        : true,
        "order"             : [],
        "ajax"              : {
            "url"   : "Hotel/tampil_rekap_hotel_kota",
            "type"  : "POST",
            "data"  : function (data) {
	...
	$('#tampilkan_rekap').on('click', function () {
		tabel_rekap_hotel_kota.ajax.reload(null, false);
	})
	...
	$('#reset_rekap').on('click', function () {
		...
		tabel_rekap_hotel_kota.ajax.reload();

		tabel_rekap_hotel_kota.clear().draw();
		tabel_rekap_hotel_kota.search("").draw();
	...
	$('#tabel_rekap_hotel_kota').on('click', '.detail-rekap', function () {
		...
		if (bln_awal == '' || bln_akhir == '') {
			window.location.href = window.location.href + "/tampil_detail_rekap_hotel_kota/"+id_kota;
		} else {
			window.location.href = window.location.href + "/tampil_detail_rekap_hotel_kota/"+id_kota+"/"+bln_awal+"/"+bln_akhir;
		}
    ...
    var tabel_rekap_hotel = $('#tabel_rekap_hotel').DataTable({
        "processing"        : true,
        "serverSide"        : true,
        "order"             : [],
        "ajax"              : {
            "url"   : base_url+"Rekap/Hotel/tampil_list_hotel",
            "type"  : "POST",
            "data"  : function (data) {
	...
	$('.detail-wisnus').on('click', function () {
		...
		var bu = base_url+"Rekap/Hotel/tampil_lihat_detail_all/wisnus/lihat/"+bln_awal+"/"+bln_akhir;
	...
	$('.detail-wisman').on('click', function () {
		...
		var bu = base_url+"Rekap/Hotel/tampil_lihat_detail_all/wisman/lihat/"+bln_awal+"/"+bln_akhir;
			...
			success: function (data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					html += '<tr>' +
						...
						'<button  class="btn btn-rose btn-sm det" data="'+data[i].nama_hotel+'" data-id="'+data[i].id_hotel+'" '+hidden+'>Detail</button>' +
	...
	$('#data-hotel').on('click','.det',function() {
		var hotel 	= $(this).attr('data');
		var id 		= $(this).attr('data-id');
		$.ajax({
			type		: "POST",
			url			: "Hotel/get_wisnus_p_hotel",

$(document).ready(function () {
	...
	$('#tabel_kota').on('click','.det',function() {
		...
		$.ajax({
			type		: "POST",
			url			: "Hotel/get_detail_kota_hotel",
			...
			success: function (data) {
				...
				$('#id_kota').val(id);
				$('#label_hotel').text(': '+data.nama_kota);
		...
		$.ajax({
			...
			success		: function (data) {
				...
				if (data.jml_wn != null) {
					...
					// $('#label_hotel').text('Nama DTW : '+hotel);
	...
	// menampilkan list hotel
    var tabel_hotel = $('#tabel_hotel').DataTable({
        "processing"        : true,
        "serverSide"        : true,
        "order"             : [],
        "ajax"              : {
	...
	$('#tabel_hotel').on('click','.det',function() {
		var id 		= $(this).data('id');
		
		$.ajax({
			type		: "POST",
			url			: "Hotel/get_detail_jml_hotel",
			data		: {id:id, periode:'awal'},
			dataType	: "json",
			success: function (data) {
				...
				$('#id_hotel').val(id);
				$('#label_hotel').text(': '+data.nama_hotel);
				$('a[href="#menu2"]').tab('show');

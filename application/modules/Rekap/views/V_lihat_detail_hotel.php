...
<head>
    ...
    <title>Rekap Hotel | <?= ucwords($nama_kota) ?></title>
...
<body>
    ...
    <?php if ($kondisi == 'lihat'): ?>
        <form method="POST" target="_self" id="tab" action="<?= base_url('Rekap/Hotel/tampil_lihat_detail/unduh') ?>">
    ...
    <?php if ($jenis_w == 'rekap_wisnus_hotel') : ?>
    ...
    <h5 style="font-weight: bold;">Hotel <?= ucwords($nama_kota) ?></h5>
    ...
    <table border="1" width="100%">
        <thead class="thead-light">
            <tr>
                ...
                <?php endforeach ?>
                <th><?= ($jenis_w == 'rekap_wisnus_hotel') ? 'Wisatawan Nusantara' : 'Wisatawan Mancanegara' ?></th>
            </tr>
        </thead>
        <tbody>
            <?php $no=1; foreach ($hotel as $d): ?>
                <tr>
                    ...
                    <?php $tot_wisnus=0; foreach ($bulan as $c): ?>
                        ...
                        <?php $hasil = $this->M_hotel->get_hotel_rekap_bulan($c, $jenis_w, $d['id_hotel'])->row_array(); ?>
